...
import { adminSteamId } from '../../config';

const router = Router();

async function verifySteamToken(req: Request, res: Response, next: NextFunction): Promise<void> {
  const token = req.cookies?.token;
  const authData = await authenticateToken(token);

  if (!authData.valid || !authData.payload?.steamId || authData.payload.steamId !== adminSteamId) {
...
router.get('/keys', verifySteamToken, async (req: Request, res: Response): Promise<void> => {
  const keys = await getApiKeys();
  res.json(keys);
});

router.post('/keys', verifySteamToken, async (req: Request, res: Response): Promise<void> => {
...
router.delete('/keys/:id', verifySteamToken, async (req: Request, res: Response): Promise<void> => {
  const db = await loadDb();
...
router.put('/keys/:id', verifySteamToken, async (req: Request, res: Response): Promise<void> => {
  if (!req.body || (!req.body.label && !req.body.expiresIn)) {
  ...
  if (req.body.expiresIn) {
    key.exp = Date.now() + req.body.expiresIn;
  }
